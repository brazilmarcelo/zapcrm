...
const FOLLOW_UP_PROMPT = `Você é {assistant_name}, assistente virtual da operação {brand_name}, enviando uma mensagem de follow-up no WhatsApp.
...
export async function generateFollowUpMessage(
  ...
  const reservationInstruction = bookingLink
    ? `Mencione o link de reserva: ${bookingLink}`
    : 'Se fizer sentido, diga que a equipe enviará o link oficial de reserva.';

  const context = followUp.context as Record<string, string>;
  const prompt = FOLLOW_UP_PROMPT
    .replace('{assistant_name}', config.agent_name || brandRuntime.assistantName)
    .replace('{brand_name}', brandRuntime.brandName)
    ...
    .replace('{reservation_instruction}', reservationInstruction)
    .replace('{memories}', memoriesText);
